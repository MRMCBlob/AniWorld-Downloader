...
def _from_request():
    ...
    return (request.args.get("apikey") or "").strip()
...
def authenticate(admin_endpoints):
    ...
    # Backward compatibility for unattended installs that already keep one
    # static admin key in Dokploy (ANIWORLD_API_KEY or *_FILE). New v5 database
    # keys continue to work alongside it and are preferred for scoped access.
    from .api_auth import configured_api_key, has_valid_api_key

    if configured_api_key() and has_valid_api_key():
        g.api_key = {
            "id": None,
            "name": "environment",
            "prefix": raw[:10],
