...
QUEUE_STATUSES = ("queued", "running", "paused", "completed", "failed", "cancelled")
...
_SCHEMA = (
    ...
    """,
    "CREATE INDEX IF NOT EXISTS idx_webhook_due ON webhook_outbox (delivered_at, next_attempt_at)",
)
...
def init_queue_db():
    """Backward-compatible name for the pre-v5 queue initializer."""
    global _initialized
    _initialized = False
    init_db()


def _rebuild_legacy_queue(conn):
    """Upgrade older queue CHECK constraints without losing rows."""
    row = conn.execute(
    ...
    logger.info("Migrating the pre-v5 download queue schema")
    conn.execute("ALTER TABLE download_queue RENAME TO download_queue_legacy_v4")
    conn.execute(
        """
    ...
    legacy_columns = {
        item["name"]
        for item in conn.execute("PRAGMA table_info(download_queue_legacy_v4)")
    }
    copied = [name for name in target_columns if name in legacy_columns]
    expressions = []
    for name in copied:
        if name == "status":
            expressions.append(
                "CASE status "
                "WHEN 'downloading' THEN 'queued' "
                "WHEN 'verifying' THEN 'queued' "
                "WHEN 'paused' THEN 'paused' "
                "WHEN 'imported' THEN 'completed' ELSE status END AS status"
            )
        else:
            expressions.append(name)
    conn.execute(
        f"INSERT INTO download_queue ({', '.join(copied)}) "
        f"SELECT {', '.join(expressions)} FROM download_queue_legacy_v4"
    )
    conn.execute("DROP TABLE download_queue_legacy_v4")
...
def add_to_queue(
    ...
    with session() as conn:
        cur = conn.execute(
            ...
            " custom_path_id, source, discord_user_id, max_attempts, priority, media_type) "
            "VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
            (
                title,
                series_url,
...
_QUEUE_ORDER = (
    ...
    "CASE WHEN status IN ('running', 'queued', 'paused') THEN position END ASC, "
...
_QUEUE_STATUSES = QUEUE_STATUSES
...
_QUEUE_SLIM_COLUMNS = (
    ...
    "position, priority, current_episode, current_url, errors, custom_path_id, source, "
    "captcha_url, discord_user_id, cancel_requested, force_cancelled, "
    "attempts, max_attempts, next_attempt_at, last_error, media_type, import_status, "
...
def is_series_queued_or_running(series_url):
    ...
    with session() as conn:
        row = conn.execute(
            ...
            "WHERE series_url = ? AND status IN ('queued','running','paused')",
            (series_url,),
        ).fetchone()
        return row["n"] > 0
...
def is_copy_queued_or_running(series_url, language, custom_path_id=None):
    ...
    with session() as conn:
        row = conn.execute(
            ...
            "AND custom_path_id IS ? AND status IN ('queued','running','paused')",
...
def _truncate_error(error, limit=1000):
    if error is None:
        return None
...
def default_max_attempts():
    try:
        return max(1, int(os.environ.get("ANIWORLD_MAX_RETRIES", "") or 1))
    except ValueError:
        return 1
...
def retry_backoff_seconds(attempt):
    try:
        base = max(1, int(os.environ.get("ANIWORLD_RETRY_BACKOFF_BASE", "") or 60))
    except ValueError:
        base = 60
    return min(base * (2 ** max(0, int(attempt) - 1)), 3600)


def schedule_queue_retry(queue_id, delay_seconds, error=None):
    """Requeue a failed item after a capped exponential-backoff delay."""
    with session() as conn:
        row = conn.execute(
        ...
        maximum = row["max_attempts"] or default_max_attempts()
        if attempts >= maximum:
            conn.execute(
...
def requeue_item(queue_id):
    with session() as conn:
        ...
        if not item or item["status"] not in ("completed", "failed", "cancelled"):
            return False
        conn.execute(
            "UPDATE download_queue SET status = 'queued', errors = '[]', "
            "current_episode = 0, current_url = NULL, started_at = NULL, "
            "completed_at = NULL, cancel_requested = 0, force_cancelled = 0, "
            "captcha_url = NULL, attempts = 0, next_attempt_at = NULL, "
            "last_error = NULL, import_status = NULL WHERE id = ?",
            (queue_id,),
        )
    return True
...
def move_queue_item(queue_id, direction):
    ...
    with session() as conn:
        ...
        neighbour = _row(
            ...
            f"AND priority = (SELECT priority FROM download_queue WHERE id = ?) "
            f"AND position {comparison} ? ORDER BY position {order} LIMIT 1",
            (queue_id, item["position"]),
        )
        if not neighbour:
...
# ---------------------------------------------------------------------------
# Durable outgoing-webhook outbox
# ---------------------------------------------------------------------------
def init_webhook_db():
    """Create the webhook outbox independently for callers used in tests."""
    with session() as conn:
        conn.execute(
            """
            CREATE TABLE IF NOT EXISTS webhook_outbox (
...
def mark_webhook_delivered(webhook_id):
    with session() as conn:
        conn.execute(
            "UPDATE webhook_outbox SET delivered_at = datetime('now'), "
...
def pause_queue_item(queue_id):
    with session() as conn:
        ...
        if not item or item["status"] != "queued":
            return False, "Only queued items can be paused"
        conn.execute(
            "UPDATE download_queue SET status = 'paused' WHERE id = ?", (queue_id,)
        )
    return True, None
