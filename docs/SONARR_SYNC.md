# Sonarr Wanted -> AniWorld/SerienStream sync
...
finds an exact title match on AniWorld and then SerienStream, and places the
matching episode directly in the series' real season directory. There is no
completed/download hand-off directory for these jobs. When the queue item
finishes, AniWorld Downloader runs
...
SONARR_SYNC_SITES=aniworld,sto
...
`SONARR_SYNC_SITES` defines the lookup order. The supported values are
`aniworld` and `sto`. An exact title/alternate-title match is selected
automatically. The sync never
...
```json
{
  ...
  "sonarr:52": "https://serienstream.to/serie/the-mentalist",
